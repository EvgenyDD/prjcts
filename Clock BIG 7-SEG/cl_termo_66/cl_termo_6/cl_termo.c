...
// Автор Дмитрий Мосин.
// Версия для шести индикаторов.
// Адаптация NBO ( Панченко Александр ).
...
{//mask_next_regim( maska regima, if (key F_), if (key SET_) )
	...
	if (regim==8) { mask_next_regim(0xFFFF,9,reg_termostat); return;}//termostat
	...
	//CALLS//****************************
	 ...
	 if (regim==34) {//set min calls
		...
		if (budilnik_nomer<kolvo_osn) a=35; else a=45;
...
static void view_in_indikator(unsigned char view){
	...
	if 			(view==0)	clock_in_lcd_gl(0);// hour-min
	else 	if 	(view==1)	hh_mm_ss(&hour[h_sek]);//hh.min.sek
	else 	if	(view==2)	clock_day_str();//hour.min day
	else 	if	(view==3)	num_day();//num mon day
	else	if	(view==4)	one_tempir_0_in_lsd();//  -000 * 1датчик
	else	if	(view==5)	one_tempir_1_in_lsd();//  -000 * 2датчик
	else	if	(view==6)	tempir_2_in_lcd(false);// -00 -00//
	else	if	(view==7)	one_tempir_0();//  -00 *C 1датчик
	else	if	(view==8)	one_tempir_1();//  -00 *C 2датчик
...
void data_lcd(void)
{
	...
	if  ((regim>=reg_termostat+1)&&(regim<=reg_termostat+3)){
		...
		if (!bcd_num3)
			lcd_buffer[3]	=0;		
				if ( PIN_OUT & PIN_T_STAT ) lcd_buffer[5]=font[n1];
	...
	if ((regim>26)&&(regim<29)){//C., //0x39,
		load_str_in_lcdbuffer((prog_uint8_t *)str_corr);
		if ( corr_flag&_BV(fl_corr_znak_pl) ) lcd_buffer[1]=font[f_seg_d]; 
	   	  else 							   	 lcd_buffer[1]=font[f_seg_g];   
	...
	//--SIGNAL--
	if (regim==216){// S, I, G, H, A, L //0,6
		load_str_in_lcdbuffer((prog_uint8_t *)str_signal);
		return;
	    }
	if (regim==52){// b, u, d, I, I. //0,5 
	...
	if (regim==218){//R, E, S, E, t //0,9
		load_str_in_lcdbuffer((prog_uint8_t *)str_default);
		return;
	    }
	if (regim==211){//P, o, r, t, 0, 0 //0,4
	...
	if (regim==213){//у see	//{}//0, Y, 0, S, E, E, 0 //1,6 
	...
	if (hour[h_min]==60)
		...
		if (hour[h_hour]==24)
			...
			if (hour[h_num]>=num_in_month[ hour[h_month] ] )
				...
				hour[h_num]=0;
				hour[h_month]++;
				if (hour[h_month]>=12) 	{hour[h_month]=0;}
				}
			}
