...
int main() {
	uchar i, button_release_counter = 0, button_release_counter2 = 0, state = STATE_WAIT, state2 = STATE_WAIT;
	
	PORTD = (1 << PD0) | (1 << PD1); // dip switch
...
/*    usbDeviceDisconnect(); // enforce re-enumeration
    ...
    while(1) {
		...
		if(!(PIND & 1<<0))
		{
			static uint8_t prevState = 0, prevState2 = 0;
			
			uchar btn = PINB & (1<<PB7);
			
			if(btn != prevState)
			{ // button pressed (PB7 at ground voltage)
			...
			{ // button pressed (PB7 at ground voltage)
				// also check if some time has elapsed since last button press
				if(state2 == STATE_WAIT && button_release_counter2 == 255)
					state2 = STATE_SEND_KEY;
					
				button_release_counter2 = 0; // now button needs to be released a while until retrigger
			} 
			prevState2 = btn;
		}
		else
		...
		if(button_release_counter2 < 255) 
			button_release_counter2++; // increase release counter
        ...
        if(usbInterruptIsReady() && state != STATE_WAIT){
			switch(state) {
			case STATE_SEND_KEY:
				keyboard_report.keycode[0] = KEY_KPASTERISK;
		...
		else if(usbInterruptIsReady() && state2 != STATE_WAIT){
			switch(state2) {
			case STATE_SEND_KEY:
				keyboard_report.keycode[0] = KEY_SPACE;
				state2 = STATE_RELEASE_KEY; // release next
				break;
			case STATE_RELEASE_KEY:
				keyboard_report.keycode[0] = 0;
				state2 = STATE_WAIT; // go back to waiting
				break;
			default:
				state2 = STATE_WAIT; // should not happen
			}
